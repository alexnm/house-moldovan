...
The English journal (`/`) and the Romanian hiking portal (`/ro`) share one **unified foundation** for body type, nav/footer shell, heading scale, and theme — but use **different display fonts**: Big Shoulders Display on EN, Oswald on RO. RO-specific behavior (difficulty accents, hike card kickers) is documented in [DESIGN-RO.md](DESIGN-RO.md).
...
A deep film-night warm neutral base with one global Film Sun accent and four saturated regional chapter colors. All values are authored in OKLCH in `src/styles/app.css`.
...
**Display Font:** Big Shoulders Display Variable — EN journal  
**Display Font (RO):** Oswald Variable — Romanian hiking portal only (`html[lang="ro"]` swaps `--font-display`)  
...
**Character:** Condensed travel-poster display (EN) with a warm humanist body. RO titles use Oswald's trail-sign register. Body prose caps at **68ch** (`--container-prose`).

### Hierarchy

- **Display / Page title** (EN: 700, `clamp(2.75rem–6.5rem)`, line-height 0.9, tracking −0.04em): `.page-title`, mixed case.
- **Headline / Section** (700, `clamp(2rem–3.25rem)`, line-height 0.94): EN `.section-heading`. Region chapter names use `.region-name` (uppercase).
...
**The Poster Owns Place Names Rule.** Uppercase display is reserved for `.region-name`. Section headings and story titles stay mixed case.

**RO display & accents.** On `/ro`, display resolves to Oswald; chapter colors encode **difficulty** (jade / saffron / terracotta). See [DESIGN-RO.md](DESIGN-RO.md).
