export const en = {
  ...
  region: {
    ...
    mapHeading: "Find destinations",
    mapHint: "Choose a country to see the locations",
    backToRegion: (name: string) => `Back to ${name}`,
    placesIn: (name: string) => `Places in ${name}`,
    noPlacesYet: "No places catalogued yet.",
    placeCount: (n: number) => (n === 1 ? "1 place" : `${n} places`),
    storiesHeading: (name: string) => `Notes from ${name}`,
