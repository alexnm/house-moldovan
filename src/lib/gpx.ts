...
export const renderProfileSvg = (
  ...
  layout: typeof ELEVATION_PROFILE_LAYOUT = ELEVATION_PROFILE_LAYOUT,
): string => {
  ...
  const hitRect = `<rect class="elev-hit" x="${innerL.toFixed(1)}" y="${innerT.toFixed(1)}" width="${innerW.toFixed(1)}" height="${innerH.toFixed(1)}" fill="transparent" cursor="crosshair" />`;
...
<text class="elev-readout-dist" fill="var(--color-ink-dim)" font-size="10" font-family="var(--font-mono)" dominant-baseline="hanging"></text>
  ...
  return [
    `<svg viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" role="img" aria-label="Elevation profile">`,
    gridLines,
    yLabels,
    xLabels,
